...
There is no `go:generate` step in this codebase — it was removed deliberately. One package still uses `go:embed` to bundle a data file that is gitignored (never committed — see [remote](#network-access--remote) below for why), so a fresh checkout embeds nothing until a maintainer manually places the file before building: [iers/iers.go](iers/iers.go) embeds `iers/data/finals2000A.all` (IERS EOP data) if present. `catalog/openngc` no longer embeds anything — it fetches over the network like every other catalog provider (see [catalog/openngc/openngc.go](catalog/openngc/openngc.go)).
...
See the README's "Data downloads & offline usage" section for the user-facing picture (sizes, `remote.SetOffline`, `iers.LoadFS`).
