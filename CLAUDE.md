...
go test -run TestCIRSToObserved ./coord/...      # single test by name
...
- **`remote/file`** — byte-addressable resources with a stable identity, size and range semantics (SPK kernels, IERS bulletins, catalog CSVs, GeoTIFF bundles), on `io/fs`.
...
**Both subpackages are internal to `remote` and nothing outside `remote/` may import them** — `TestSubpackagesAreNotImportedDirectly` in `internal/docsguard` fails on any such import, tests included. They stay separate packages because the split above is real, but `remote` is the only door: [remote/frontdoor.go](remote/frontdoor.go) re-exports everything they do (`FS`, `File`, `OpenFS`, `Open`, `WriteFile`, `RemoveFile`, `Schemes`, the `Client` request methods, `HTTPError`, `RetryPolicy`). A backend for a further scheme would be one more such subpackage, blank-imported and exporting nothing; none currently ships.

That boundary is the point of the arrangement rather than tidiness. The subpackages move bytes; `remote` decides whether bytes may move — which endpoint, whether the process is offline, whether a download was consented to, where the result is cached. Going around it produces working code that silently ignores `SetOffline` at one call site.

**Policy is a value, not a process global.** `remote.Client` holds offline mode, download consent, endpoint URL overrides and the cache location; `remote.NewClient()` builds one and every package-level function is a method on `remote.Default()` (the `http.DefaultClient` analogue). Add a new policy control as a `*Client` method plus a one-line package-level delegation — never as a new package-level variable.
...
- **No scheme is ever hardcoded in `remote` or `remote/file`.** `remote/file` owns a scheme-dispatched registry populated from each backend's `init`; do not build a second one (no `Backend`/`Source`/`Transport` interface, no `if scheme == …`, no per-scheme branch in the fetch path). An `Endpoint.URL` is the **exact string** handed to `file.OpenFS`. Core `remote/file` compiles in `file://`, `http://`, `https://` and `mem://`, and **those are the only schemes that currently resolve** — `s3://`, `gs://`, `azblob://` and `sftp://` went with gocloud.dev and have not been replaced ([docs/storage.md](docs/storage.md) records why; `remote.CopernicusEODATA` is the one endpoint affected). A backend for any of them is an opt-in subpackage that **contains** its implementation and **exports nothing**, registering from `init`, so its SDK stays out of every build that does not use it. Adding one means editing nothing in `remote`.
- **Scheme-specific connection detail rides in the URL's query string, never in Go.** A backend's opener parses what it needs from the URL — `region`, `endpoint`, `use_path_style` for S3 — so reaching a non-AWS S3 service needs no astrogo configuration API and no AWS SDK type in any signature. `file.OpenFS` applies two portable wrappers on every scheme at once, after the backend is built: `?prefix=sub/dir/` scopes a filesystem (through `fs.Sub`), and `?key=exact/object.dat` serves one object under any name — the supported way to point an endpoint at a single file, since a bare single-object URL leaves no room for the caller's `name`.
- **Nothing is assumed local.** There is no `LocalURL`/`OSPath`/`SetDataDirPath`, and **no API anywhere takes a `path string` meaning an OS filesystem path**. Every signature is a `remote.FS` + name, or a filesystem URL. Names are `/`-separated and must satisfy `fs.ValidPath` — use `path.Join`, never `filepath.Join`. Nothing needs `os.MkdirAll`: a "directory" is only a key prefix. The single OS-path contact point in the module is the unexported default-cache-dir resolver in [remote/storage.go](remote/storage.go), which turns `os.UserCacheDir()` into a `file://` URL; `ASTROGO_CACHE_DIR` holds a URL, not a path. The deliberate `os.*` exceptions are `fits.Open` (an arbitrary user-supplied FITS file, not astrogo-managed data) and the `os.CreateTemp` staging in `catalog/fink` and `atmosphere/dataset/cams`, where a third-party decoder demands a real OS path — that staging is the consumer's problem at its own call site, never pushed back into `remote`'s signatures.
- **Bulk file downloads never happen without explicit consent.** A missing kernel makes `jpl.NewProvider`/`eph.NewProvider` (both take `ctx` first) fail with an actionable `remote.ErrDownloadDenied` unless the caller granted `remote.EnableDownloads(maxSize, ids...)` — with no ids it covers every `Downloadable` endpoint. Consent is checked twice per fetch: against the registered `ApproxSize` before any request, then against the size the source actually reports. Never add an implicit download; route it through `remote.GetFile`. The one deliberate *automatic* (not unconsented) behavior is `time`'s EOP lazy load: `Time.EOP()`/`.UTC()`/`.UT1()` trigger `iers.EnsureLoaded`, which reads a pre-seeded cache object unconditionally but still gates its *network* step behind the same consent check. That lazy load only happens at all once something blank-imports **`remote/eop`** — the loader lives there because it needs `remote.GetFile` and so cannot live in `remote` itself, which imports it nowhere. Without that import every epoch silently uses zero DUT1 and polar motion; measured on the Horizons corpus, the topocentric gap goes from a p50 of 1.4 arcsec against a 3 arcsec contract to a max of 8.8 arcsec. Any astrogo test asserting sub-arcsecond topocentric accuracy must carry the import.
...
- **All API access goes through `remote.Client`'s own `Get`/`GetJSON`/`PostForm`/`PostJSON`** — never a raw `http.Client`, and never `remote/api` directly. `remote.Default()` is the process-wide one; a component wanting its own timeouts, pacing or token takes `remote.Default().Clone()` and calls `SetAPIOptions`. Each method resolves `id` through `URL(id)` at request time (so offline mode and `Disable` reach a client that already exists) and uses a transport built per endpoint with that endpoint's registered `Timeout` (`remote.DefaultAPITimeout` if zero). All return `io.ReadCloser` or decode directly, never `*http.Response`, since a non-2xx becomes an `*remote.HTTPError` before a caller sees a body. **No resty type escapes the package** and `Client` has no exported fields — tests redirect an endpoint with `httptest.NewServer` + `remote.SetURL` rather than injecting a transport. Retries use resty's own `RetryConditionStatus*` predicates (429, 5xx except 501, status 0); note that resty's `SetRetryDefaultConditions` covers only transport/header/URL errors, so status retrying **must** be added explicitly.
- **Reuse `remote.GetFile` instead of hand-rolling "ensure cached, then read/download".** `GetFile(ctx, id, name, opts...) (fsys remote.FS, key string, err error)` resolves source and cache through the same opener, reuses a cached object on existence alone for `Mutable: false` endpoints (JPL kernels) or after a recorded-source-ETag check for `Mutable: true` ones (IERS, OpenNGC), and on a miss downloads under a cross-process lock built on `O_CREATE|O_EXCL`. It returns filesystem+key, not a stream — the caller reads it however it needs. `WithCacheName` sets the cache key when it differs from the source name (IERS serves `finals2000A.all`, cached as `finals2000A.data`); `WithValidate(func(io.Reader) error)` runs against the **staged** object before promotion, so a corrupt fetch is never cached and a multi-GB kernel never has to fit in memory. `remote.WriteFile` is the generic write primitive for content that doesn't arrive via `GetFile` (a decoded API payload, a checksum sidecar), and a missing object is `errors.Is(err, fs.ErrNotExist)` — there is no astrogo-specific predicate for it.
...
- **Random access uses `remote.Open(ctx, fsys, name)`**, which returns a `remote.File` (`fs.File` + `io.ReaderAt` + `io.Seeker`) with a chunk cache in front of it: aligned chunks kept in a bounded LRU (64 KiB × 16 = **1 MiB resident for any object**, a 3 GB kernel included; it does not buffer the object). Sequential `Read` and `Seek` pass straight through, so a whole-object download is still one request. The cache is inside `Open` rather than beside it because every backend already has a `ReadAt` and using it directly is quadratically wrong — measured over 2000 SPK-shaped reads of a 64 MiB object: `file://` 4.5 ms uncached against 53 µs chunked, and `http` **11.4 s and 2000 requests** uncached against a server on *localhost*. Do not "simplify" it away without re-running `BenchmarkChunkedReadAt` and the backend benchmarks beside it in `remote/file/fsys_bench_test.go`.
...
- **`time`** is the sole gateway for Earth Orientation Parameters and epoch arithmetic. `time/internal/iers` (unexported — nothing outside `time/` can import it) fetches/parses IERS EOP data; `time` re-exports what's needed (`time.EOP`, `time.RegisterModel`/`GetModel`/`Coverage`/`SetRetryCooldown`) and adds `Time.EOP()`, `Time.MJD()`, `Time.GAST()`, `Time.JulianEpochYear()`, `Time.DayOfYear()`. EOP data loads lazily the first time `Time.EOP()`/`Time.UTC()`/`Time.UT1()` needs it — a pre-seeded on-disk cache file, then (if `remote.EnableDownloads(remote.IERSFinals2000A, ...)` was called) a network fetch, then a zero-EOP-plus-one-time-warning degradation — with no explicit populate call, but only once a blank import of `remote/eop` has registered the loader. `coord` and every other package get EOP/epoch values through these `time` APIs — never by hand-rolling MJD/GAST arithmetic or importing EOP internals directly.
