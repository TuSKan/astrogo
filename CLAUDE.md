...
skybrightness/atlas, skybrightness/lpmap                 ← light-pollution data providers (downloaded atlases; live API)
...
- **`skybrightness`** (+ `skybrightness/atlas`, `skybrightness/lpmap`) models night-sky surface brightness (moonlight, zodiacal light, airglow, light-pollution floor) as additive linear-flux components. `lpmap` is a live client for the lightpollutionmap.info point-query API (needs a manually-issued key). `atlas` decodes light-pollution atlas files (GeoTIFF/HDF5) AND owns the consent-gated downloads for them (`EnsureWorldAtlas`/`EnsureVIIRSAnnual` via `remote.WorldAtlas`/`remote.VIIRSAnnual`) AND composes every source behind one `atlas.Resolver`/`atlas.FloorAt` — pick an `atlas.Layer` (`LayerWorldAtlas`, `LayerVIIRS`, `LayerLightPollutionMap`, `LayerBortle`, `LayerScalar`, or `LayerAuto`) and it resolves a `skybrightness.Floor`, reporting what it tried in `Result.Attempts`. `LayerAuto`'s ladder is **freshness-first** (VIIRS, newest published year via `NewestVIIRSYear`, before the 2015-frozen World Atlas) — deliberate, and not the same as fidelity-first: the World Atlas is the radiative-transfer-propagated source and VIIRS is a raw-radiance empirical fit, so a caller who needs fidelity asks for `WithLayer(LayerWorldAtlas)` rather than relying on the ladder. Don't reorder `autoOrder` without updating its comment, the `LayerAuto` doc, and the order-pinning resolver tests together. `atlas` may import `lpmap` (it does, for `LayerLightPollutionMap`); neither is ever imported back by core `skybrightness` (enforced by `skybrightness/importgraph_test.go`). Note `atlas` pulls in `github.com/scigolib/hdf5` + `archive/zip`, so do NOT import it from `plan` — that would put an HDF5 library in every `plan` user's build; `plan` talks to core `skybrightness` only, via `LimitingMagnitudeConstraint`/`ScoreObservableSky`.
