...
- `remote.Transport`/`remote.RegisterTransport` — `GetFile`'s byte-transfer step is now dispatched by `Endpoint.Kind` through a small registered-transport table instead of being hardcoded to `net/http`; `remote`'s two built-in kinds (`KindAPI`/`KindFile`) keep their existing behavior unchanged via a new internal `httpTransport`. A caller wanting a different `Kind` registers its own `Transport` explicitly — `remote` never links a protocol-specific SDK into a build that doesn't use one. New `remote.KindS3`, `Endpoint.Bucket`, and `remote.CopernicusEODATA` (the Copernicus Data Space Ecosystem's S3-compatible `eodata` bucket) register the addressing shape for an S3-backed endpoint without pulling any S3 client into `remote` itself.
- New package `remote/s3` — the `Transport` implementation for `KindS3` endpoints, and the only importer of the AWS SDK v2 and `github.com/ungerik/go-fs/s3fs` in this module (mirroring the existing `scigolib/hdf5`-scoped-to-`skybrightness/dataset/granule` isolation pattern). `s3.Register(ctx, id, opts...)` resolves credentials exclusively through the AWS SDK v2's standard default chain (env vars, `~/.aws/credentials`, or an IAM role) — astrogo never reads a credential file of its own, and never reads `AWS_ENDPOINT_URL` (the service endpoint is the registry's own `remote.URL(id)`). `FetchInto`/`Probe` read an object's body exclusively through `gofs.File.OpenReader()` (i.e. `s3fs`, the same interface every other file backend in this codebase goes through — `remote` never assumes a "local file" and never special-cases S3 with a raw SDK body read), compensating in-package for `s3fs`'s confirmed leading-slash key bug (a `s3URIFor` helper builds `"s3://"+bucket+key` with no separating slash, so `s3fs`'s own `CleanPathFromURI` trims to the correct unprefixed key). The raw `*s3.Client` is kept for exactly one purpose — `HeadObject` metadata calls (`Probe`'s ETag, which `s3fs.Stat` cannot provide at all, and `FetchInto`'s pre-download exact-size consent check, needed because `s3fs.OpenReader` does HEAD+GET as one atomic, uninterruptible call) — never for an object's body. `s3fs@v0.1.0` has no streaming read path in this dependency version (confirmed by reading its source: every read, large or small, ends in a full in-memory buffer), so `FetchInto` does not bound peak memory below the object's own size — a deliberate, documented tradeoff in exchange for never hand-rolling a second body-transfer implementation alongside `gofs.File`'s own.
