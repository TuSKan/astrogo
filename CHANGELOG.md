...
- New package `remote/s3` — the `Transport` implementation for `KindS3` endpoints, and the only importer of the AWS SDK v2 and `github.com/ungerik/go-fs/s3fs` in this module (mirroring the existing `scigolib/hdf5`-scoped-to-`skybrightness/dataset/granule` isolation pattern). `s3.Register(ctx, id, opts...)` resolves credentials exclusively through the AWS SDK v2's standard default chain (env vars, `~/.aws/credentials`, or an IAM role) — astrogo never reads a credential file of its own, and never reads `AWS_ENDPOINT_URL` (the service endpoint is the registry's own `remote.URL(id)`). `FetchInto`/`Probe` call the SDK's `GetObject`/`HeadObject` directly rather than routing through `s3fs`'s own File API, which — confirmed by reading its source — builds the wrong object key from a `gofs.File("s3://...")` URI (a leading-slash bug) and buffers whole objects into memory with no streaming path; `remote/s3` instead streams `GetObjectOutput.Body` straight into `remote.Save`, keeping peak memory bounded by the copy buffer rather than the object size even for a ~180 MB CAMS aerosol tracer file. `atmosphere/dataset/cams`'s NetCDF reader (Sky Brightness V2 Phase 3/7) is the deliberate next phase, not included here.
