...
- `skybrightness.ScatteredIn` implements Masana et al. (2024) Eq. 11, the hemispheric scattering integral, over `atmosphere.SingleScatteredRadiance` and `atmosphere.CombinedPhaseFunction`; `Model.AboveAtmosphere` supplies its incoming field by dividing out the extended-source factor the natural components applied. Measured at the Table 2 geometry, the full model closes 34 per cent of the starlight-to-zodiacal gap the simplified transfer cannot touch at all.
