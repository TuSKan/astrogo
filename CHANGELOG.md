...
- `atmosphere` gains `State`/`Builder`/`NewBuilder()`/`Aerosol`/`CloudLayer`/`SurfaceOptical`/`HorizonProfile`/`StandardDefault` and the general data-provenance primitives `SourceRef`/`Fidelity`/`TimeRange`/`DatasetVersion` — the atmospheric state a `skybrightness.Request` evaluates under, reusable by any future atmosphere-aware constraint (weather, seeing), not sky-brightness-specific. `skybrightness` aliases the provenance primitives for its own short in-package names but references `atmosphere.State` directly, matching how `coord.Context` is already used.
...
- `plan.LimitingMagnitudeConstraint` is rewritten against the new `Engine`/`Passband` API (`Engine`/`Passband`/`Conversion` fields replace `Model`/`Conversion`, `Atmosphere` is now `*atmosphere.State`); `plan` continues to import core `skybrightness` only, never a subpackage (machine-enforced by `skybrightness/importgraph_test.go`).
