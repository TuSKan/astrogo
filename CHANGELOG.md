...
- `ephemeris/kepler` (new package) — a network-free alternative to SPK-kernel-backed ephemerides: `kepler.Elements`/`Elements.StateAt`/`SolveKepler` propagate a position directly from classical heliocentric osculating orbital elements via two-body Keplerian motion, and `kepler.Provider`/`New`/`WithBase` adapt that into a full `ephemeris/core.Provider`. Re-exported as `ephemeris.Elements`/`NewFromElements`/`WithKeplerBase`, and wrapped by `plan.NewAsteroidFromElements` so a Kepler-propagated body drops straight into `plan.VisibleTonight`/`ObservableWindows` like any kernel-backed one. Elliptical orbits only (`0 <= e < 1`); no planetary perturbations, so accuracy drifts away from the elements' epoch — validated live against 433 Eros's real published elements and JPL Horizons' real (perturbed) ephemeris, measuring ~0.04″ divergence near epoch growing to ~0.56″ at ±30 days.
- `examples/22_kepler_propagator` — resolves 1 Ceres's real osculating elements live from JPL SBDB via `catalog.NewResolver(catalog.SBDB)`, propagates with no SPK kernel, then runs the result through `plan.VisibilityEvents` exactly like any other target.
