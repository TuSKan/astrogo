...
### Added
- **The integrated-starlight map is published**: `starmap-o8-V-total.txt.gz`, 5.5 MB, reachable through `starlight.Open` and the `remote.GaiaStarMap` endpoint at release tag `starmap-v1`. HEALPix order 8 in Johnson V on GAMBONS’ own grid, all 1,811,709,771 Gaia DR3 sources with no magnitude cut, colourless sources recovered rather than dropped, plus the 74 Hipparcos stars Gaia saturates on — 6.4 per cent of the flux, and 70 of them brighter than V = 3. Measured at 21.91 mag arcsec⁻² in the Galactic plane and 23.87 at the poles, 22.77 whole-sky. Fetched end to end through `starlight.Open` before release. [#NN]
