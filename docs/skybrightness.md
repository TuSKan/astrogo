...
transmission functions or the cloud-opacity term can be recovered from `∝`.

Only **one** of those four is actually required: Kocifaj (2007), the paper that put clouds
into the model. The rest add axes — planar sources in 2008, scattering orders above the
first in 2018, which that paper's own abstract calls marginal below 30 km. §16 has the
reasoning and what would unblock it.
...
| **SkyGlow Simulator governing equations** | Phase 5 outright | **One paper, not four: Kocifaj (2007), *Appl. Opt.* **46**, 3013–3022.** Its title is the model — "Light-pollution model for cloudy and cloudless night skies with ground-based light sources" — and it is where `L₁`, `L₂` and `L∞` acquire their constants, their transmission functions and the form of `Ψ(h,θₕ)`. Paywalled, checked. Institutional access or an author preprint unblocks Phase 5. <br><br>The other three the PNAS paper cites are refinements along different axes, and the paper's own wording separates them: the solution was "first introduced in 2007 (ref. 14) and 2008 (ref. 36), and later improved in 2017 (ref. 20)", while "in 2018 (ref. 37), the radiative transfer model was used as a generating function for the calculation of higher-scattering-order radiance components". So Kocifaj (2008), *Appl. Opt.* **47**, 792 generalises the source to planar geometry, which `GroundEmitter` may or may not need; and the 2018 JQSRT paper adds scattering orders above the first, which its own abstract calls marginal — "multiple scattering usually plays a marginal role at distances below 30 km, and single scattering is an appropriate approximation" — over exactly the 0 to 18.5 km the PNAS study spans. <br><br>**Why the newest does not simply supersede the oldest.** This lineage branches by capability rather than replacing itself: each paper adds an axis and none restates the base model. The newest of all, PNAS 2025, is a results paper that points backwards for its equations. There is therefore no single latest paper that contains the model, and 2007 is load-bearing for the reason that it is the one that put clouds in. <br><br>Until it is in hand Phase 5 stays unimplemented rather than approximated — a universal cloud multiplier is on §2's prohibited list precisely because it cannot reproduce the amplification-and-screening sign reversal that is this component's acceptance criterion. |
