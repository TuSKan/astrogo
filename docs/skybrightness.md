...
| 5 | Clouds (Kocifaj 2007 + 2025) | **Unblocked, not yet built.** `atmosphere` already carries the scene vocabulary (`CloudLayer`, `CloudPhase`, `CloudMorphology`) and `skybrightness` already has the `UnknownCloud` quality flag, so a scene can describe a cloud field it cannot yet evaluate. Kocifaj (2007) supplies the governing equations the PNAS 2025 results paper only sketched — Eq. 16 the scattering angle at altitude, Eq. 18 the angular scattering distribution, Eq. 24 the closed total intensity with its cloud-reflection and below-cloud-scattering terms. What remains is code: cloud optical properties in `atmosphere`, a `Component`, and the 2025 paper's extension of a single reflecting layer to a fractional 3D cloud field (`CF`, the opacity `ο(z,A)`, and the above-cloud term `L₂`). See §11.2 |
