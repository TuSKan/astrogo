...
| **VizieR V/50 position columns** | Publishing an R-band starlight map | One query. `AddCousinsR` matches the Bright Star Catalogue positionally on `RAJ2000`/`DEJ2000`, the standard VizieR computed-position columns, and that is the one part of the multi-band bright-star path not confirmed against the live service — VizieR returned "TAP service too busy" and 503 throughout the session it was written in. `I/239/hip_main`'s `Vmag`, `B-V` and `V-I` and `V/50/catalog`'s `R-I` **are** confirmed live; `HD` on `hip_main` is confirmed *absent*, which is why the match is positional. Until it runs, R is the one band whose bright stars are unverified. |
| **A HEAD-less fetch in `httpblob`** | The local SFD dust map, and with it a sky map that does not query IRSA 20,000 times | One condition in `gocloud-ext`. Harvard Dataverse, which archives the SFD maps under doi:10.7910/DVN/EWCNL5, answers **403 to HEAD** while serving ranged GET normally. `httpblob.headObject` already carries the right fallback — a one-byte ranged GET whose `Content-Range` reports the full size — but reaches it only on `405 Method Not Allowed`, so a 403 bails first. Everything else is in place and tested: the endpoint is registered, the ZEA projection is implemented and covered (including an explicit equal-area check), and `TestSFDMatchesIRSA` is ready to compare against 1,979 answers already cached from IRSA, so validating it costs that service nothing. |
