...
| Dwarf planet magnitude — Pluto | ✅ validated | `magnitude/planets_horizons_test.go` | JPL Horizons APmag | 0.02 mag | Pluto with Charon, from the Explanatory Supplement (1992) Table 7.48.1: V(1,0) = −1.01 (Harris 1961), 0.041 mag/° (Binzel & Mulholland 1984). Horizons uses −1.00, so 5 dates 2020–2026 sit 0.010–0.012 mag bright, against a rotational variation of 0.1–0.3 mag. Until #407 `PlanetApparent` listed Pluto and returned `ErrUnsupportedBody`, and `VisibleTonight` dropped the error, so Pluto never appeared at any limit |
