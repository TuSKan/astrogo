...
| Astronomical time scales | ✅ validated | `time.scale.roundtrip.arithmetic`, `time.scale.roundtrip.modelled` | gofa / SOFA | 1e-12 d | UTC ↔ TAI ↔ TT ↔ TDB verified in `time`, as round trips, which cannot see an error in the TDB−TT model itself: that is the next row. **Note the scope:** this row covers `time`'s own conversions and never covered `ephemeris/jpl/lsk`, which parses the NAIF leap-second kernel independently — and which dropped the final table entry, so every UTC epoch after 2017-01-01 converted **one second early** and put the geocentric Sun ~30 km from DE440. Fixed, with an offline regression test that also guards the next leap second; the gap was invisible because nothing compared the two paths against each other |
| TDB − TT | ✅ validated | `time/tdb_test.go` | SOFA `iauDtdb`: Fairhead & Bretagnon (1990), all 787 terms, ±3 ns over 1950–2050 against DE405 | 1 µs over 1600–2400; 3 µs over −1000 to 3000 | The leading 37 terms, as they stand in `iauDtdb`'s table: **0.79 µs** at worst over 1900–2100, 0.92 µs over 1600–2400, 2.7 µs over −1000 to 3000, at a fifth of a microsecond per conversion where `iauDtdb` takes 5.6. Until #423 five terms, only one of them FB90's, were **54 µs** off within 1900–2100 against a documented ±1 µs, and this table claimed 1e-12 d from a round trip |
