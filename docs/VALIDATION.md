...
| Lunar Eclipse (historical) | ✅ validated | `plan/nasa_eclipse_test.go` | [NASA 5MC Lunar](https://eclipse.gsfc.nasa.gov/LEcat5/LEcatalog.html) | both directions; greatest eclipse 1 min | **1452/1452** catalog eclipses found across 6 centuries (1–2000 CE), every catalog row, and **no eclipse reported that the catalog does not list**: both directions are asserted. Greatest eclipse 0.2 min from the catalog on average, 0.5 at worst, asserted at 1 min. Until #401 astrogo decided an eclipse by a fixed 1.58° latitude limit, which missed the 1958-04-04 penumbral (magnitude 0.0135) and reported 76 eclipses that do not happen, and only the catalog's direction was checked; until #398 the parser dropped the 28 rows whose type codes it did not list, and this row read 1424/1424 |
| Solar Eclipse (historical) | ✅ validated | `plan/nasa_eclipse_test.go` | [NASA 5MC Solar](https://eclipse.gsfc.nasa.gov/SEcat5/SEcatalog.html) | both directions; greatest eclipse 1 min | **1433/1433** catalog eclipses found across 6 centuries (1–2000 CE), every catalog row, and **no eclipse reported that the catalog does not list**. Greatest eclipse 0.2 min from the catalog on average, 0.5 at worst. The nearest non-eclipses sit 0.00016 Earth radii outside the limb, which takes the WGS 84 spheroid: a spherical Earth reports five of them. Until #401 the fixed latitude limit reported 97 eclipses that do not happen; until #398 the parser dropped 50 rows (hybrids, non-central eclipses, the first and last of a saros) and this row read 1383/1383 |
