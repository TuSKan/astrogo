// Package vizier provides a resolve.ConeSearcher implementation targeting the
// CDS VizieR catalog service.
//
// VizieR is an astronomical catalog registry hosting thousands of datasets.
// Utilizing ADQL spatial geometries, this provider executes asynchronous Cone Searches
// across datasets (e.g., UCAC4, 2MASS) mapping point-source locations dynamically.
package vizier
