...
func TestDataDirBucketURLLeavesAMalformedURLAlone(t *testing.T) {
	...
	if got := Default().dataDirBucketURL(); got != malformed {
...
// TestEachClientStagesInItsOwnCache is the interaction #266 and #269 have with
// each other, which neither on its own covers.
//
// #266 made the cache URL carry no_tmp_dir=1 so fileblob stages inside the
// bucket. #269 made the cache location a property of a Client rather than of
// the process. Merging them has one wrong answer that compiles and passes both
// PRs' own tests: resolving the staging URL through the package-level
// DataDirURL, which reads Default. Every client would then stage into — and
// read from — the default's cache, and a component given its own cache
// directory would silently share the process one.
func TestEachClientStagesInItsOwnCache(t *testing.T) {
	// Not parallel: it reads Default.
	t.Cleanup(func() { SetDataDir("") })

	SetDataDir("file:///default-cache")

	own := NewClient(WithCacheURL("file:///own-cache"))

	got := own.dataDirBucketURL()

	if !strings.Contains(got, "own-cache") {
		t.Errorf("the client opens %q, which is not the cache it was given.\n"+
			"  A client's cache location has to reach the URL it actually opens; "+
			"resolving through the package-level DataDirURL sends every client to "+
			"Default's cache (#266, #269).", got)
	}

	if !strings.Contains(got, "no_tmp_dir=1") {
		t.Errorf("the client opens %q, without the staging parameter.\n"+
			"  A caller-supplied cache URL needs it as much as the default one does — "+
			"that is the half of #241 that would otherwise be correct only for people "+
			"who configured nothing.", got)
	}

	if d := Default().dataDirBucketURL(); !strings.Contains(d, "default-cache") {
		t.Errorf("Default now opens %q.\n"+
			"  Constructing a client with its own cache must not move the process one.", d)
	}
}
